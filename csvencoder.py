import csv, random, operator, subprocess
import numpy as np
...
class Music:
    def __init__(self, key):
        ...
        self.major = "major"
        self.bpm = 120
        self.tempo = 60000000 / self.bpm # 500000 = 120 quarter notes (beats) per minute; also 60,000,000 / bpm
    ...
    # Song setup
    def start(self):
        self.lines.append([0, 0, 'Header', self.midi_type, self.num_tracks, self.quarter_note])
...
#----------------------------------------------------------------------------------------------
    ...
    def major_135_chord(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 4, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 7, 0])
    ...
    def minor_135_chord(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time, 'Note_on_c', ch, note + 7, 80])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 3, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 7, 0])
    ...
    def major_135_chord_inv1(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 4, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 12, 0])
    ...
    def major_135_chord_inv2(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 12, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 16, 0])
    ...
    def minor_135_chord_inv2(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 12, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 15, 0])
...
#------------------------------------------------------------------------------------------------
    ...
    def minor_15_chord(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', 0, note, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', 0, note + 7, 0])
...
#-------------------------------------------------------------------------------------------------
    ...
    def major_135_chord_arpeg(self, note, length, ch):
        self.lines.append([1, self.LH_time, 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_on_c', ch, note + 4, 80])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_off_c', ch, note + 4, 0])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_on_c', ch, note + 7, 80])
        self.lines.append([1, self.LH_time + length, 'Note_off_c', ch, note + 7, 0])
    ...
    def minor_135_chord_arpeg(self, note, length, ch):
        self.lines.append([1, self.LH_time, 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_on_c', ch, note + 3, 80])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_off_c', ch, note + 3, 0])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_on_c', ch, note + 7, 80])
        self.lines.append([1, self.LH_time + length, 'Note_off_c', ch, note + 7, 0])
    ...
    def tonic_158_chord_arpeg(self, note, length, ch):
        self.lines.append([1, self.LH_time, 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.LH_time + (length / 4), 'Note_on_c', ch, note + 7, 80])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_on_c', ch, note + 12, 80])
        self.lines.append([1, self.LH_time + length, 'Note_off_c', ch, note + 12, 0])
    ...
    def modified_tonic_158_chord_arpeg(self, note, length, ch):
        ...
        self.lines.append([1, self.LH_time + (length / 2), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (length / 2), 'Note_on_c', ch, note + 12, 80])
        self.lines.append([1, self.LH_time + (3 * length / 4), 'Note_off_c', ch, note + 12, 0])
        self.lines.append([1, self.LH_time + (3 * length / 4), 'Note_on_c', ch, note + 14, 80])
        self.lines.append([1, self.LH_time + length, 'Note_off_c', ch, note + 14, 0])
    ...
    def fast_358_chord_arpeg(self, note, length, ch):
        self.lines.append([1, self.LH_time, 'Note_on_c', ch, note + 4, 80])
        self.lines.append([1, self.LH_time + (length / 32), 'Note_off_c', ch, note + 4, 0])
        self.lines.append([1, self.LH_time + (length / 32), 'Note_on_c', ch, note + 7, 80])
        self.lines.append([1, self.LH_time + (length / 16), 'Note_off_c', ch, note + 7, 0])
        self.lines.append([1, self.LH_time + (length / 16), 'Note_on_c', ch, note + 12, 80])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, note + 12, 0])
...
#-------------------------------------------------------------------------------------------------
    ...
    def jazzy_rhythm_note_top(self, note, length, ch):
        ...
        self.lines.append([1, self.RH_time + (length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (2 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (3 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (4 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (5 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (6 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (7 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (8 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (9 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (10 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (11 * length / 16), 'Note_off_c', ch, note, 0])

        self.lines.append([1, self.RH_time + (23 * length / 32), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (12 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (25 * length / 32), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (13 * length / 16), 'Note_off_c', ch, note, 0])
        self.lines.append([1, self.RH_time + (14 * length / 16), 'Note_on_c', ch, note, 80])
        self.lines.append([1, self.RH_time + (15 * length / 16), 'Note_off_c', ch, note, 0])
...
#-------------------------------------------------------------------------------------------------
    ...
    def shuffled_chord_progs(self, amount, chord_length, key, ch):
        ...
        options = [
            (self.block_prog_1465, [1, chord_length, key, ch]),
...
#-------------------------------------------------------------------------------------------------
    ...
    def truly_random_melody(self, amount, length, key, ch):
        ...
        for _ in range(amount):
            ...
            self.lines.append([1, self.misc_time1 + (7 * length / 8), 'Note_off_c', ch, note, 0])
    ...
    def tonic_major_melody(self, note, amount, length, ch):
        ...
        for _ in range(amount):
            ...
            self.lines.append([1, self.RH_time + (7 * length / 8), 'Note_off_c', ch, new_note, 0])
    ...
    def holy_is_the_lord_melody(self, key, ch):
        ...
        for i in range(2):
            ...
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 77 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 120 / 8), 'Note_off_c', ch, 77 + key, 0])
            self.misc_time2 += 120
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 76 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 74 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 120 / 8), 'Note_off_c', ch, 74 + key, 0])
            self.misc_time2 += 120
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 72 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 120 / 8), 'Note_off_c', ch, 72 + key, 0])
            self.misc_time2 += 120
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 72 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 840 / 8), 'Note_off_c', ch, 72 + key, 0])
            self.misc_time2 += 840

            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 79 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 480 / 8), 'Note_off_c', ch, 79 + key, 0])
            self.misc_time2 += 480
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 77 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 77 + key, 0])
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 76 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 76 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 480 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 480
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 74 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 1200 / 8), 'Note_off_c', ch, 74 + key, 0])
            self.misc_time2 += 1200
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 72 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 72 + key, 0])
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 76 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 720 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 720
            ...
            self.misc_time2 += 240
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 77 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 480 / 8), 'Note_off_c', ch, 77 + key, 0])
            self.misc_time2 += 480
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 76 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * 240 / 8), 'Note_off_c', ch, 76 + key, 0])
            self.misc_time2 += 240
            ...
            self.misc_time2 += 480
            self.lines.append([1, self.misc_time2, 'Note_on_c', ch, 74 + key, 80])
            self.lines.append([1, self.misc_time2 + (7 * (720 + (i * 480))/ 8), 'Note_off_c', ch, 74 + key, 0])
    ...
    def jazzy_prog(self, amount, length, key, ch):
        for _ in range(amount):
            self.repeat(self.jazzy_rhythm_note_top, [60 + key, length / 2, ch], 3) # RH
    ...
    def xmen_full(self, amount, length, key, ch):
        ...
        for _ in range(amount):
            ...
            for i in range(2):
                for note in notes:
                    self.lines.append([1, self.misc_time1, 'Note_on_c', ch + 2, note, 80])
                    self.lines.append([1, self.misc_time1 + length / 2, 'Note_off_c', ch + 2, note, 0])
                    self.misc_time1 += length / 2
                
                # First iteration of the loop
                if i is 0:
                    self.lines.append([1, self.misc_time1, 'Note_on_c', ch + 2, 55 + key + 12, 80])
                    self.lines.append([1, self.misc_time1 + length / 2, 'Note_off_c', ch + 2, 55 + key + 12, 0])
                    self.misc_time1 += length / 2
                    self.lines.append([1, self.misc_time1, 'Note_on_c', ch + 2, 57 + key + 12, 80])
                    self.lines.append([1, self.misc_time1 + length / 2 * 3, 'Note_off_c', ch + 2, 57 + key + 12, 0])
                    self.misc_time1 += length / 2 * 3
                
                # Second iteration of the loop
                else:
                    self.lines.append([1, self.misc_time1, 'Note_on_c', ch + 2, 53 + key + 12, 80])
                    self.lines.append([1, self.misc_time1 + length, 'Note_off_c', ch + 2, 53 + key + 12, 0])
                    self.misc_time1 += length
                    self.lines.append([1, self.misc_time1, 'Note_on_c', ch + 2, 52 + key + 12, 80])
                    self.lines.append([1, self.misc_time1 + length, 'Note_off_c', ch + 2, 52 + key + 12, 0])
                    self.misc_time1 += length
    ...
    def ending(self, key, length, ch):
        ...
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, 36 + key, 0])
        self.lines.append([1, self.LH_time + (7 * length / 8), 'Note_off_c', ch, 48 + key, 0])
        self.LH_time += length
...
# 
# TODO: Remove this from this file and make it a separate
# executable from the command-line. Also clean up the 
# actual methods that generate the melodies.
def main():
    song = Music(0)
    song.start()
